...
insert into hdban values('mahd 3','2/5/2019','ten 3')
...
begin
	declare @soluong int
	declare @slton int
	select @soluong = hangban.soluong from hangban inner join inserted on hangban.mahd = inserted.mahd
	select @slton = hang.slton from hang inner join inserted on hang.mahang = inserted.mahang
	if (@soluong > @slton)
		begin
		...
		begin
			update hang set slton = slton - @soluong
			from hang inner join inserted on hang.mahang = inserted.mahang
		end
end
		select * from hang
		select * from hangban
		insert into hangban values('mahd 3','mahang 1',2000,5)
		select * from hang
		select * from hangban
